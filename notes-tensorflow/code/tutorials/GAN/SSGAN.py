...
class GAN(object):
  ...
  def dis_dcn(self, x_image, training=False):
    ...
    with tf.variable_scope('fc'):
      ...
      # fc
      outputs = tf.layers.dense(reshape, self.y_dim+1, name='outputs')
    return outputs
  ...
  def build_model(self):
    ...
    # 真实图像送入判别器
    d_logits_r,d_real = self.discriminator(self.x, reuse=False,if_train=self.if_train)
    # 生成图像送入辨别器
    d_logits_f,d_fake = self.discriminator(self.G_img, reuse=True,if_train=self.if_train)

    self.d_loss ,self.g_loss = self.get_loss(d_logits_r,d_logits_f,self.label)

    self.accuracy = self.eval_metric(self.label,d_logits_r)
    ...
    if self.if_train:
      ...
      self.global_step = ckpt.model_checkpoint_path.split('/')[-1].split('-')[-1]
      print('model load done')


  def get_loss(self,d_logits_r, d_logits_f, label):
    # label=k+1 维，第k+1维=1，表示假样本，否则表示真样本
    real_label = tf.concat([label, tf.zeros([self.batch_size, 1])], axis=1)
    fake_label = tf.concat([tf.zeros([self.batch_size, self.y_dim]), tf.ones([self.batch_size, 1])], axis=1)

    # Discriminator/classifier loss
    d_loss_real = tf.nn.softmax_cross_entropy_with_logits(logits=d_logits_r, labels=real_label)
    d_loss_fake = tf.nn.softmax_cross_entropy_with_logits(logits=d_logits_f, labels=fake_label)
    d_loss = tf.reduce_mean(d_loss_real + d_loss_fake)
    g_loss = -tf.reduce_mean(tf.log(tf.clip_by_value(1-d_logits_f[:,-1], 1e-10, 1.0)))

    return d_loss, g_loss


  def eval_metric(self,label,pred):
    # 计算正确率
    correct_prediction = tf.equal(tf.argmax(label, 1), tf.argmax(pred[:, :-1], 1))
  ...
  def eval(self,epoch_i):
    ...
    batch_x = batch_x.reshape(-1, self.img_dim[0], self.img_dim[1], self.img_dim[2])
    accuracy_score = self.sess.run(self.accuracy, feed_dict={self.x: batch_x, self.label: batch_y})
    print("After %s training epoch(s), validation accuracy = %g" % (epoch_i, accuracy_score))
